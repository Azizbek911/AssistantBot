...
bot.action("adminpanel", async (ctx) => {
    ...
    if (isAdminOrOwner(userID)) {
        const adminPanelButtons = [
            ...
            [{ text: "Guruhlar 👥", callback_data: "groups_list" }]
        ]
...
bot.on('callback_query', async (ctx) => {
    ...
    try {
        ...
        if (data === "view_admins" && ctx.chat.type === 'private') {
            ...
            currentOperation[userID].messageId = sentMessage.message_id;
        } else if (data === "groups_list" && ctx.chat.type === "private") {
            ...
            if (buttons.length != 0) {
                return ctx.editMessageText('Choose a group:', Markup.inlineKeyboard(buttons));
            } else{
                ctx.answerCbQuery("Guruhlar kiritlmagan 🚫")
            }
            ...
            GroupIDOnly = group.id;
            currentOperation[userID] = { type: "groups_list", messageId: null }
            const sentMessage = await ctx.editMessageText("Guruh uchun xabar kiriting: ", { reply_markup: { inline_keyboard: [[{ text: "Bekor qilish", callback_data: "cancel" }]] } })
            currentOperation[userID].messageId = sentMessage.message_id;
        }
    } catch (error) {
...
bot.on('message', async (ctx) => {
    ...
    if (currentOperation[userID]) {
        ...
        try {
            ...
            } else if (operation.type === "groups_list") {
                try {
                    await bot.telegram.forwardMessage(GroupIDOnly, ctx.message.chat.id, ctx.message.message_id);
                    await ctx.reply("Xabar jo'natildi ✅")
                } catch (error) {
                    console.log("Biz xatolik mavjud: ", error);
                }
            }
        } catch (error) {
            console.log("Bizda xatolik mavjud: ", error);
